...
#ifndef IOX_HOOFS_VM_IPC_VM_SHARED_MEMORY_HPP
#define IOX_HOOFS_VM_IPC_VM_SHARED_MEMORY_HPP
...
enum class VMSharedMemoryError : uint8_t
{
    EMPTY_NAME,
    INVALID_FILE_NAME,
...
//class VMSharedMemory : public FileManagementInterface<VMSharedMemory>
class VMSharedMemory
{
  public:
    ...
    VMSharedMemory(const VMSharedMemory&) = delete;
    VMSharedMemory& operator=(const VMSharedMemory&) = delete;
    VMSharedMemory(VMSharedMemory&&) noexcept;
    VMSharedMemory& operator=(VMSharedMemory&&) noexcept;
    ~VMSharedMemory() noexcept;
    ...
    static expected<bool, VMSharedMemoryError> unlinkIfExist(const Name_t& name) noexcept;
    ...
    friend class VMSharedMemoryBuilder;

  private:
    VMSharedMemory(const Name_t& name, const shm_handle_t handle, const bool hasOwnership, uint64_t size) noexcept;
    ...
    static VMSharedMemoryError errnoToEnum(const int32_t errnum) noexcept;

    //friend struct FileManagementInterface<VMSharedMemory>;
...
class VMSharedMemoryBuilder
{
    ...
    ///        file system
    IOX_BUILDER_PARAMETER(VMSharedMemory::Name_t, name, "")
  ...
  public:
    ...
    expected<VMSharedMemory, VMSharedMemoryError> create() noexcept;
};

} // namespace detail
} // namespace iox

#endif // IOX_HOOFS_VM_IPC_VM_SHARED_MEMORY_HPP
