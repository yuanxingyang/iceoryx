...
#ifndef IOX_HOOFS_POSIX_IPC_VIRTIO_VSOCK_INL
#define IOX_HOOFS_POSIX_IPC_VIRTIO_VSOCK_INL
...
namespace iox
{
template <typename Type, VirtioVSock::Termination Terminator>
expected<void, PosixIpcChannelError> VirtioVSock::timedSendImpl(not_null<const Type*> msg,
                                                                     uint64_t msgSize,
                                                                     const units::Duration& timeout) const noexcept
{
    ...
    {
        IOX_LOG(ERROR, "sending on server side not supported for virtio vsock \"" << m_name << "\"");
...
template <typename Type, VirtioVSock::Termination Terminator>
expected<uint64_t, PosixIpcChannelError> VirtioVSock::timedReceiveImpl(
    not_null<Type*> msg, uint64_t maxMsgSize, const units::Duration& timeout) const noexcept
{
    ...
    {
        IOX_LOG(ERROR, "receiving on client side not supported for virtio vsock \"" << m_name << "\"");
        return err(PosixIpcChannelError::INTERNAL_LOGIC_ERROR);
    }
...
template <uint64_t N>
expected<void, PosixIpcChannelError> VirtioVSock::send(const iox::string<N>& buf) const noexcept
{
...
template <uint64_t N>
expected<void, PosixIpcChannelError> VirtioVSock::timedSend(const iox::string<N>& buf,
                                                                 const units::Duration& timeout) const noexcept
{
...
template <uint64_t N>
expected<void, PosixIpcChannelError> VirtioVSock::receive(iox::string<N>& buf) const noexcept
{
...
template <uint64_t N>
expected<void, PosixIpcChannelError> VirtioVSock::timedReceive(iox::string<N>& buf,
                                                                    const units::Duration& timeout) const noexcept
{
