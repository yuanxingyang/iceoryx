...
#ifndef IOX_HOOFS_POSIX_IPC_ETH_SOCKET_INL
#define IOX_HOOFS_POSIX_IPC_ETH_SOCKET_INL
...
namespace iox
{
template <typename Type, EthSocket::Termination Terminator>
expected<void, PosixIpcChannelError> EthSocket::timedSendImpl(not_null<const Type*> msg,
                                                                     uint64_t msgSize,
                                                                     const units::Duration& timeout) const noexcept
{
    ...
    {
        IOX_LOG(Error, "sending on server side not supported for eth socket \"" << m_name << "\"");
...
template <typename Type, EthSocket::Termination Terminator>
expected<uint64_t, PosixIpcChannelError> EthSocket::timedReceiveImpl(
    not_null<Type*> msg, uint64_t maxMsgSize, const units::Duration& timeout) const noexcept
{
    ...
    {
        IOX_LOG(Error, "receiving on client side not supported for eth socket \"" << m_name << "\"");
        return err(PosixIpcChannelError::INTERNAL_LOGIC_ERROR);
    }
...
template <uint64_t N>
expected<void, PosixIpcChannelError> EthSocket::send(const iox::string<N>& buf) const noexcept
{
...
template <uint64_t N>
expected<void, PosixIpcChannelError> EthSocket::timedSend(const iox::string<N>& buf,
                                                                 const units::Duration& timeout) const noexcept
{
...
template <uint64_t N>
expected<void, PosixIpcChannelError> EthSocket::receive(iox::string<N>& buf) const noexcept
{
...
template <uint64_t N>
expected<void, PosixIpcChannelError> EthSocket::timedReceive(iox::string<N>& buf,
                                                                    const units::Duration& timeout) const noexcept
{
