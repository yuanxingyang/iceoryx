...
#ifndef IOX_HOOFS_POSIX_IPC_VIRTIO_VSOCK_HPP
#define IOX_HOOFS_POSIX_IPC_VIRTIO_VSOCK_HPP
...
namespace iox
{
class VirtioVSockBuilder;

static constexpr size_t LONGEST_VALID_NAME = 20;
...
class VirtioVSock
{
  public:
    ...
    /// @brief The name length is limited by the size of the sockaddr_vm::sun_path buffer and the IOX_SOCKET_PATH_PREFIX

    using Builder_t = VirtioVSockBuilder;

    using VSockName_t = string<LONGEST_VALID_NAME>;
    using Message_t = string<MAX_MESSAGE_SIZE>;

    VirtioVSock() noexcept = delete;
    VirtioVSock(const VirtioVSock& other) = delete;
    VirtioVSock(VirtioVSock&& other) noexcept;
    VirtioVSock& operator=(const VirtioVSock& other) = delete;
    VirtioVSock& operator=(VirtioVSock&& other) noexcept;

    ~VirtioVSock() noexcept;
    ...
    static expected<bool, PosixIpcChannelError> unlinkIfExists(const VSockName_t& name) noexcept;

    /// @brief unlink is not applicable to VSOCK
    /// @param NoPathPrefix signalling that this method does not add a path prefix
    /// @param name of the unix domain socket to unlink
    /// @return true if the unix domain socket could be unlinked, false otherwise, PosixIpcChannelError if error occured
    static expected<bool, PosixIpcChannelError> unlinkIfExists(const NoPathPrefix_t, const VSockName_t& name) noexcept;
  ...
  private:
    friend class VirtioVSockBuilderNoPathPrefix;

    VirtioVSock(const VSockName_t& name,
    ...
    static PosixIpcChannelError errnoToEnum(const VSockName_t& name, const int32_t errnum) noexcept;

    expected<void, PosixIpcChannelError> closeFileDescriptor() noexcept;
    static expected<void, PosixIpcChannelError> closeFileDescriptor(const VSockName_t& name,
                                                                    const int sockfd,
                                                                    const sockaddr_vm& sockAddr,
...
class VirtioVSockBuilder
{
    ...
    IOX_BUILDER_PARAMETER(uint64_t, maxMsgSize, VirtioVSock::MAX_MESSAGE_SIZE)

    /// @brief Defines the max number of messages for the socket.
    IOX_BUILDER_PARAMETER(uint64_t, maxMsgNumber, VirtioVSock::MAX_NUMBER_OF_MESSAGES)

  public:
    /// @brief create a unix domain socket
    /// @return On success a 'VirtioVSock' is returned and on failure an 'PosixIpcChannelError'.
    expected<VirtioVSock, PosixIpcChannelError> create() const noexcept;

    inline bool isValidVSockName(const VSockName_t& name) const noexcept
    {
...
#include "detail/virtio_vsock.inl"

#endif // IOX_HOOFS_POSIX_IPC_VIRTIO_VSOCK_HPP
