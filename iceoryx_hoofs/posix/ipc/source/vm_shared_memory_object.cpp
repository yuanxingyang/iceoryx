...
constexpr const void* const VMSharedMemoryObject::NO_ADDRESS_HINT;
...
expected<VMSharedMemoryObject, VMSharedMemoryObjectError> VMSharedMemoryObjectBuilder::create() noexcept
{
    auto printErrorDetails = [this] {
        ...
        IOX_LOG(Error,
                "Unable to create a vm shared memory object with the following properties [ name = "
    ...
    auto sharedMemory = detail::VMSharedMemoryBuilder()
                            .name(m_name)
    ...
    if (!sharedMemory)
    {
        ...
        return err(VMSharedMemoryObjectError::SHARED_MEMORY_CREATION_FAILED);
    }
    ...
    if (!realSizeResult)
    {
        ...
        IOX_LOG(Error,
                ...
                "underlying object.");
        return err(VMSharedMemoryObjectError::UNABLE_TO_VERIFY_MEMORY_SIZE);
    }
    ...
    if (realSize < m_memorySizeInBytes)
    {
        ...
        return err(VMSharedMemoryObjectError::REQUESTED_SIZE_EXCEEDS_ACTUAL_SIZE);
    }
    */

    auto memoryMap = detail::VMMemoryMapBuilder()
                         ...
                         .accessMode(m_accessMode)
                         .flags(detail::VMMemoryMapFlags::SHARE_CHANGES)
                         .offset(0)
                         .create();
    ...
    if (!memoryMap)
    {
        ...
        return err(VMSharedMemoryObjectError::MAPPING_SHARED_MEMORY_FAILED);
    }
    ...
    if (sharedMemory->hasOwnership())
    {
        ...
        if (platform::IOX_SHM_WRITE_ZEROS_ON_CREATION)
        {
            ...
            if (memsetSigbusGuard.has_error())
            {
                ...
                return err(VMSharedMemoryObjectError::INTERNAL_LOGIC_FAILURE);
            }
    ...
    return ok(VMSharedMemoryObject(std::move(*sharedMemory), std::move(*memoryMap)));
}

VMSharedMemoryObject::VMSharedMemoryObject(detail::VMSharedMemory&& sharedMemory,
                                                 detail::VMMemoryMap&& memoryMap) noexcept
    : m_sharedMemory(std::move(sharedMemory))
    , m_memoryMap(std::move(memoryMap))
{
}

VMSharedMemoryObject::VMSharedMemoryObject(VMSharedMemoryObject&& rhs) noexcept
: m_sharedMemory(std::move(rhs.m_sharedMemory))
, m_memoryMap(std::move(rhs.m_memoryMap))
{
}

VMSharedMemoryObject& VMSharedMemoryObject::operator=(VMSharedMemoryObject&& rhs) noexcept
{
    m_sharedMemory = std::move(rhs.m_sharedMemory);
    m_memoryMap = std::move(rhs.m_memoryMap);
}

const void* VMSharedMemoryObject::getBaseAddress() const noexcept
{
    return m_memoryMap.getBaseAddress();
}

void* VMSharedMemoryObject::getBaseAddress() noexcept
{
    return m_memoryMap.getBaseAddress();
}

shm_handle_t VMSharedMemoryObject::get_file_handle() const noexcept
{
    return m_sharedMemory.getHandle();
}

shm_handle_t VMSharedMemoryObject::getFileHandle() const noexcept
{
    return m_sharedMemory.getHandle();
}

expected<uint64_t, FileStatError> VMSharedMemoryObject::get_size() const noexcept
{
    return m_sharedMemory.get_size();
}

bool VMSharedMemoryObject::hasOwnership() const noexcept
{
