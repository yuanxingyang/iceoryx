...
namespace detail
{
expected<VMMemoryMap, VMMemoryMapError> VMMemoryMapBuilder::create() noexcept
{
#ifdef __ANDROID_VM_SHM__
    ...
    if (result)
    {
        return ok(VMMemoryMap(result.value().value, m_length));
    }
    ...
    return err(VMMemoryMap::errnoToEnum(result.error().errnum));
#else
    void* address = reinterpret_cast<void*>(mmap_device_io(m_length,reinterpret_cast<uintptr_t>(m_baseAddressHint)));
    if(address)
    {
        return ok(VMMemoryMap(reinterpret_cast<void *>(address), m_length));
    }
    constexpr uint64_t FLAGS_BIT_SIZE = 32U;
    IOX_LOG(Error,
            "Unable to map memory with the following properties [ baseAddressHint = "
                ...
                << ", flags = " << std::bitset<FLAGS_BIT_SIZE>(static_cast<uint32_t>(m_flags)).to_string()
                << ", offset = " << iox::log::hex(m_offset) << " ]");
    return err(VMMemoryMap::errnoToEnum(EPERM));
#endif
}

VMMemoryMap::VMMemoryMap(void* const baseAddress, const uint64_t length) noexcept
...
VMMemoryMapError VMMemoryMap::errnoToEnum(const int32_t errnum) noexcept
{
    switch (errnum)
    {
    case EACCES:
    ...
    case EINVAL:
        ...
        return VMMemoryMapError::INVALID_PARAMETERS;
    case ENFILE:
        IOX_LOG(Error, "System limit of maximum open files reached");
        return VMMemoryMapError::OPEN_FILES_SYSTEM_LIMIT_EXCEEDED;
    case ENODEV:
        IOX_LOG(Error, "Memory mappings are not supported by the underlying filesystem.");
        return VMMemoryMapError::FILESYSTEM_DOES_NOT_SUPPORT_MEMORY_MAPPING;
    case ENOMEM:
        IOX_LOG(
            Error,
            "One or more of the following failures happened:\n"
        ...
        return VMMemoryMapError::NOT_ENOUGH_MEMORY_AVAILABLE;
    case EOVERFLOW:
        IOX_LOG(Error, "The sum of the number of pages and offset are overflowing. (only 32-bit architecture)");
        return VMMemoryMapError::OVERFLOWING_PARAMETERS;
    case EPERM:
        IOX_LOG(Error,
                "One or more of the following failures happened:\n"
        ...
        return VMMemoryMapError::PERMISSION_FAILURE;
    case ETXTBSY:
        IOX_LOG(Error, "The memory region was set up with MAP_DENYWRITE but write access was requested.");
        return VMMemoryMapError::NO_WRITE_PERMISSION;
    default:
        IOX_LOG(Error, "This should never happened. An unknown error occurred!\n");
        return VMMemoryMapError::UNKNOWN_ERROR;
    };
}

VMMemoryMap::VMMemoryMap(VMMemoryMap&& rhs) noexcept
{
    *this = std::move(rhs);
}

VMMemoryMap& VMMemoryMap::operator=(VMMemoryMap&& rhs) noexcept
{
    if (this != &rhs)
    {
        if (!destroy())
...
VMMemoryMap::~VMMemoryMap() noexcept
{
    if (!destroy())
    {
...
const void* VMMemoryMap::getBaseAddress() const noexcept
{
    return m_baseAddress;
}

void* VMMemoryMap::getBaseAddress() noexcept
{
    return m_baseAddress;
}

bool VMMemoryMap::destroy() noexcept
{
