...
//! [subscriber callback]
void subscriberCallback(iox::popo::Subscriber<CounterTopic>* const subscriber, uint64_t* const sumOfAllSamples)
{
    //while (subscriber->hasData())
    {
        subscriber->take().and_then([&](auto& userPayload) {
            ...
            std::cout.setf(flags);
            */
           std::cout << "subscriber: " << userPayload->counter << std::endl;
        });
...
int main()
{
    ...
    iox::vector<iox::popo::Subscriber<CounterTopic>, NUMBER_OF_SUBSCRIBERS> subscriberVector;
