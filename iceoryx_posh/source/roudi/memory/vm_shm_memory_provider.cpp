...
namespace roudi
{
constexpr access_rights VMShmMemoryProvider::SHM_MEMORY_PERMISSIONS;

VMShmMemoryProvider::VMShmMemoryProvider(const ShmName_t& shmName,
...
VMShmMemoryProvider::~VMShmMemoryProvider() noexcept
{
    if (isAvailable())
    {
...
expected<void*, MemoryProviderError> VMShmMemoryProvider::createMemory(const uint64_t size,
...
expected<void, MemoryProviderError> VMShmMemoryProvider::destroyMemory() noexcept
{
    m_shmObject.reset();
    return ok();
}
